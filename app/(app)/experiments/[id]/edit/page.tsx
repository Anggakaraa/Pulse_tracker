...
function MetricTag({ metricKey, onRemove }: { metricKey: string; onRemove: () => void }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ display: "inline-block", width: "6px", height: "6px", borderRadius: "50%", backgroundColor: CATEGORY_COLORS[meta.category], flexShrink: 0 }} />
      <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "13px", color: colors.ink }}>{meta.name}</span>
      <button onClick={onRemove} style={{ background: "none", border: "none", cursor: "pointer", padding: "0 0 0 2px", color: colors.inkMuted, fontSize: "14px", lineHeight: 1, display: "flex", alignItems: "center" }}>×</button>
    </div>
  );
}
  ...
  return (
    <div ref={containerRef} style={{
      ...
      <div style={{ maxHeight: "280px", overflowY: "auto" }}>
        {grouped.length === 0 && <div style={{ padding: "14px 12px", fontFamily: "var(--font-dm-sans)", fontSize: "13px", color: colors.inkMuted }}>No match</div>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
      <div style={{ display: "flex", alignItems: "baseline", gap: "8px" }}>
        <label style={{ fontFamily: "var(--font-outfit)", fontSize: "13px", fontWeight: 600, letterSpacing: "0.08em", textTransform: "uppercase", color: colors.inkMuted }}>{label}</label>
...
export default function EditExperimentPage() {
  ...
  useEffect(() => {
    async function load() {
      const [{ data: exp }, { data: metrics }] = await Promise.all([
        supabase.from("experiments").select("*").eq("id", id).single(),
        supabase.from("experiment_metrics").select("metric_key, target_low, target_high").eq("experiment_id", id),
      ]);
      if (exp) {
        setName(exp.name ?? "");
      ...
      if (metrics) {
        setSelectedMetrics(metrics.map(m => m.metric_key));
        const t: Record<string, { low: string; high: string }> = {};
        for (const m of metrics) {
          t[m.metric_key] = {
            low: m.target_low != null ? String(m.target_low) : "",
            high: m.target_high != null ? String(m.target_high) : "",
          };
        }
        setTargets(t);
      }
  ...
  const toggleMetric = (key: string) => {
    setSelectedMetrics(prev => {
      ...
      setTargets(t => ({ ...t, [key]: t[key] ?? { low: "", high: "" } }));
      return [...prev, key];
    });
  };
  ...
  const handleSave = async () => {
    ...
    if (expErr) { setError("Failed to save. Please try again."); setSaving(false); return; }

    // Replace experiment_metrics
    await supabase.from("experiment_metrics").delete().eq("experiment_id", id);
    if (selectedMetrics.length > 0) {
      ...
      if (metricsErr) { setError("Experiment saved but metrics failed."); setSaving(false); return; }
    }
  ...
  if (loading) {
    return (
      <div style={{ fontFamily: "var(--font-dm-sans)", fontSize: "14px", fontWeight: 300, color: colors.inkMuted }}>
        Loading…
      </div>
    );
  }
  ...
  return (
    <div>
      <div style={{ marginBottom: "24px" }}>
        <Link href={`/experiments/${id}`} style={{ fontFamily: "var(--font-dm-sans)", fontWeight: 300, fontSize: "14px", color: colors.inkMuted, textDecoration: "none" }}>
          ← Experiment
        </Link>
      </div>
      ...
      <div style={{ maxWidth: "600px", display: "flex", flexDirection: "column", gap: "28px" }}>
        ...
        <Field label="Name">
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Seed oil elimination" style={inputStyle} />
        </Field>
        ...
        <Field label="Observation window">
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
              <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "12px", fontWeight: 300, color: colors.inkMuted }}>End date <span style={{ opacity: 0.6 }}>(leave blank if ongoing)</span></span>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            {(["active", "completed"] as const).map(s => (
              <button key={s} onClick={() => setStatus(s)} style={{
                ...
                border: `1px solid ${status === s ? colors.ink : colors.border}`,
                backgroundColor: status === s ? colors.ink : colors.surface,
                color: status === s ? colors.background : colors.inkMuted,
              }}>
        ...
        <Field label="Metrics to track" hint={`up to ${MAX_METRICS}`}>
          ...
          <div style={{ position: "relative" }}>
            <button onClick={() => setSelectorOpen(v => !v)} disabled={selectedMetrics.length >= MAX_METRICS} style={{
              fontFamily: "var(--font-dm-sans)", fontSize: "13px",
              color: selectedMetrics.length >= MAX_METRICS ? colors.inkMuted : colors.ink,
              backgroundColor: colors.surface, border: `1px solid ${colors.border}`,
              borderRadius: "4px", padding: "7px 14px", cursor: selectedMetrics.length >= MAX_METRICS ? "default" : "pointer",
              display: "inline-flex", alignItems: "center", gap: "6px",
            }}>
          ...
          <Field label="Targets" hint="(optional — shown in progression table)">
            <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
              {selectedMetrics.map(key => {
                ...
                return (
                  <div key={key} style={{ display: "grid", gridTemplateColumns: "1fr 100px 100px", alignItems: "center", gap: "12px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      <span style={{ display: "inline-block", width: "6px", height: "6px", borderRadius: "50%", backgroundColor: CATEGORY_COLORS[meta?.category as CategoryKey] ?? colors.inkMuted, flexShrink: 0 }} />
                      <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "13px", color: colors.ink }}>{meta?.name ?? key}</span>
                      {meta?.unit && <span style={{ fontFamily: "var(--font-dm-sans)", fontSize: "12px", fontWeight: 300, color: colors.inkMuted }}>{meta.unit}</span>}
                    </div>
                    <input type="number" value={targets[key]?.low ?? ""} onChange={e => setTarget(key, "low", e.target.value)} placeholder="Min" style={{ ...inputStyle, padding: "6px 10px" }} />
                    <input type="number" value={targets[key]?.high ?? ""} onChange={e => setTarget(key, "high", e.target.value)} placeholder="Max" style={{ ...inputStyle, padding: "6px 10px" }} />
                  </div>
                );
              })}
              <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: "12px", fontWeight: 300, color: colors.inkMuted, margin: "4px 0 0" }}>
                Leave blank to skip. Min only = ≥, Max only = ≤, both = range.
              </p>
            </div>
          </Field>
        )}

        {error && <p style={{ fontFamily: "var(--font-dm-sans)", fontSize: "13px", color: colors.badge.act, margin: 0 }}>{error}</p>}

        <div style={{ display: "flex", gap: "8px", alignItems: "center", paddingTop: "8px" }}>
          <Link href={`/experiments/${id}`} style={{
